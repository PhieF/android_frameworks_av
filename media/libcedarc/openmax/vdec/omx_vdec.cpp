                                                   ...
                                                   OMX_IN OMX_PTR         pCmdData)
{
    ...
    switch (nMsg)
    {
        case OMX_VdrvFeedbackMsg_NotifyEos:
            logi("(omx_vdec, f:%s, l:%d) send OMX_VdrvFeedbackMsg_NotifyEos",
                  __FUNCTION__, __LINE__);
            eCmdNative = VdrvNotifyEos;
            break;

        case OMX_VdrvFeedbackMsg_ResolutionChange:
            logi("(omx_vdec, f:%s, l:%d) send OMX_VdrvFeedbackMsg_ResolutionChange",
                  __FUNCTION__, __LINE__);
            eCmdNative = VdrvResolutionChange;
            break;

        default:
            logw("(omx_vdec, f:%s, l:%d) send unknown feedback message[0x%x]",
                  __FUNCTION__, __LINE__, nMsg);
...
static inline int vdecoderPrepare(omx_vdec* pSelf)
{
    ...
    {
        logw("fatal error! when vdrv OMX_VdrvCommand_PrepareVdecLib, m_state[0x%x] \
...
static inline void vdecoderDecodeStream(omx_vdec* pSelf)
{
    ...
    else if(decodeResult < 0)
    {
        ...
        pSelf->m_Callbacks.EventHandler(&pSelf->mOmxCmp, pSelf->m_pAppData,
                                        OMX_EventError, OMX_ErrorHardware,
                                        0, NULL);
        waitPipeDataToRead(pSelf->m_vdrv_cmdpipe[0], 20*1000);
        //break;
    }
    else
    {
...
static inline void sendStreamDataToDecoder(omx_vdec* pSelf)
{
    ...
    if (pInBufHdr)
    {
        if (pInBufHdr->nFlags & OMX_BUFFERFLAG_EOS)
        {
            ...
            sem_post(&pSelf->m_sem_vbs_input);
            sem_post(&pSelf->m_sem_frame_output);
            logd("(f:%s) wait for OMX_VdrvCommand_EndOfStream", __FUNCTION__);
            sem_wait(&pSelf->m_vdrv_cmd_lock);
            logd("(f:%s) wait for OMX_VdrvCommand_EndOfStream done!", __FUNCTION__);
    ...
    if(0 == nRetSemGetValue)
    {
        ...
        {
            logi("(f:%s, l:%d) Be careful, sema vbs_input[%d]!=0",
                  __FUNCTION__, __LINE__, nSemVal);
        }
    }
    else
    {
        logw("(f:%s, l:%d) fatal error, why sem getvalue of m_sem_vbs_input[%d] fail!",
             __FUNCTION__, __LINE__, nRetSemGetValue);
    }
...
static inline void drainPicture(omx_vdec* pSelf)
{
    ...
    if(pOutBufHdr == NULL)
    {
        waitPipeDataToRead(pSelf->m_cmdpipe[0], 40*1000);
        return ;
    }
...
static inline int processThreadCommand(omx_vdec* pSelf, ThrCmdType cmd, OMX_U32 cmddata)
{
...
static inline int processVdrvThreadCommand(omx_vdec* pSelf, OMX_VDRV_COMMANDTYPE cmd)
{
...
static void* ComponentThread(void* pThreadData)
{
    ...
    while (1)
    {
        ...
        if (FD_ISSET(pSelf->m_cmdpipe[0], &rfds))
        {
            ...
            if(processThreadCommand(pSelf, cmd, cmddata) == OMX_RESULT_EXIT)
                goto EXIT;
        }
...
drain_picture:
            ...
            else
            {
                ...
                waitPipeDataToRead(pSelf->m_cmdpipe[0], 20*1000);
            }
        }
    }
...
static void* ComponentVdrvThread(void* pThreadData)
{
    ...
    while (1)
    {
        ...
        if (FD_ISSET(pSelf->m_vdrv_cmdpipe[0], &rfds))
        {
            ...
            if(processVdrvThreadCommand(pSelf, cmd) == OMX_RESULT_EXIT)
                nStopFlag = 1;
            ...
            if(0 == nRetSemGetValue)
            {
                ...
                {
                    logw("(f:%s, l:%d) fatal error, why nSemVal[%d]!=0",
                         __FUNCTION__, __LINE__, nSemVal);
                }
            }
            else
            {
                logw("(f:%s, l:%d) fatal error, why sem getvalue[%d] fail!",
                     __FUNCTION__, __LINE__, nRetSemGetValue);
            }
        }
